...
const getImages = (
  fetchedPackages: TravelPackageDto,
  country: TravelCountryPackages
): string[] => {
  let result: string[] = [];
  const imagePath = `/packages/${
    country.countryCode
  }/${fetchedPackages.packageId.toLowerCase()}/`;
...
const getTravelPackages = (): TravelCountryPackages[] => {
  const data = fs.readFileSync("./public/packages/travelpackages.json", {
    encoding: "utf8"
  });
